...
public class PrimeNum {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter the number");// this is the first line to excecute in and print in console
		 ...
		 //using for loop to increment i until i<=num  and conditinal statement if and when condition are true incrementing the count  
		 for (int i = 1; i <=num ; i++) {
			if(num%i==0) {
				count++;
			}
		}
		 //when count became 2 then it will be prime number else it is not a prime number
		 if(count==2) {
